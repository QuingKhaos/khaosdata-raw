return {
  autoplace = {
    order = "a[ruin]-i[tiny]",
    placement_density = 2,
    probability_expression = "0.0005 * fulgora_oil_mask * (fulgora_elevation > -10)+ (1 - fulgora_oil_mask)* (0.005 * (0.1 * fulgora_natural_mask + fulgora_mesa)+ (0.04 + 0.01 * fulgora_decorative_machine_density)* (fulgora_artificial_mask + 0.2 * fulgora_mesa * max(0, fulgora_scrap_medium)))"
  },
  collision_box = {
    {
      -0.45,
      -0.45
    },
    {
      0.45,
      0.45
    }
  },
  collision_mask = {
    colliding_with_tiles_only = true,
    layers = {
      object = true
    }
  },
  name = "fulgoran-ruin-tiny",
  order = "a[fulgora]-b[decorative]",
  pictures = {
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 0,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 128,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 256,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 384,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 512,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 640,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 768,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 896,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 1024,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 1152,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 1280,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 1408,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 1536,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 1664,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 1792,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 1920,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 2048,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 2176,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 2304,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 2432,
      y = 0
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 0,
      y = 128
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 128,
      y = 128
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 256,
      y = 128
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 384,
      y = 128
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 512,
      y = 128
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 640,
      y = 128
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 768,
      y = 128
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 896,
      y = 128
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 1024,
      y = 128
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 1152,
      y = 128
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 1280,
      y = 128
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 1408,
      y = 128
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 1536,
      y = 128
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 1664,
      y = 128
    },
    {
      filename = "__space-age__/graphics/decorative/fulgoran-ruin/fulgoran-ruin-tiny.png",
      height = 128,
      line_length = 20,
      scale = 0.3,
      shift = {
        0,
        0
      },
      width = 128,
      x = 1792,
      y = 128
    }
  },
  type = "optimized-decorative",
  walking_sound = {
    advanced_volume_control = {
      fades = {
        fade_in = {
          curve_type = "cosine",
          from = {
            control = 0.3,
            volume_percentage = 0
          },
          to = {
            control = 0.6,
            volume_percentage = 100
          }
        }
      }
    },
    variations = {
      {
        filename = "__base__/sound/walking/pebble/stones-1.ogg",
        volume = 0.6
      },
      {
        filename = "__base__/sound/walking/pebble/stones-2.ogg",
        volume = 0.6
      },
      {
        filename = "__base__/sound/walking/pebble/stones-3.ogg",
        volume = 0.6
      },
      {
        filename = "__base__/sound/walking/pebble/stones-4.ogg",
        volume = 0.6
      },
      {
        filename = "__base__/sound/walking/pebble/stones-5.ogg",
        volume = 0.6
      },
      {
        filename = "__base__/sound/walking/pebble/stones-6.ogg",
        volume = 0.6
      },
      {
        filename = "__base__/sound/walking/pebble/stones-7.ogg",
        volume = 0.6
      },
      {
        filename = "__base__/sound/walking/pebble/stones-8.ogg",
        volume = 0.6
      },
      {
        filename = "__base__/sound/walking/pebble/stones-9.ogg",
        volume = 0.6
      }
    }
  }
}
