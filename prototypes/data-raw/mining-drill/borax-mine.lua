return {
  allowed_effects = {
    "consumption",
    "speed",
    "productivity"
  },
  allowed_module_categories = {
    "productivity",
    "speed",
    "efficiency",
    "quality"
  },
  circuit_connector = {
    {
      points = {
        shadow = {
          green = {
            -1,
            3.34375
          },
          red = {
            -0.8125,
            3.34375
          }
        },
        wire = {
          green = {
            -0.9375,
            2.90625
          },
          red = {
            -1,
            2.6875
          }
        }
      },
      sprites = {
        blue_led_light_offset = {
          -1.25,
          2.9375
        },
        connector_main = {
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04a-base-sequence.png",
          height = 50,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.6875
          },
          width = 52,
          x = 104,
          y = 150
        },
        led_blue = {
          draw_as_glow = true,
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04e-blue-LED-on-sequence.png",
          height = 60,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.65625
          },
          width = 60,
          x = 120,
          y = 180
        },
        led_blue_off = {
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04f-blue-LED-off-sequence.png",
          height = 44,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.65625
          },
          width = 46,
          x = 92,
          y = 132
        },
        led_green = {
          draw_as_glow = true,
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04h-green-LED-sequence.png",
          height = 46,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.65625
          },
          width = 48,
          x = 96,
          y = 138
        },
        led_light = {
          intensity = 0,
          size = 0.9
        },
        led_red = {
          draw_as_glow = true,
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04i-red-LED-sequence.png",
          height = 46,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.65625
          },
          width = 48,
          x = 96,
          y = 138
        },
        red_green_led_light_offset = {
          -1.25,
          2.84375
        },
        wire_pins = {
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04c-wire-sequence.png",
          height = 58,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.6875
          },
          width = 62,
          x = 124,
          y = 174
        }
      }
    },
    {
      points = {
        shadow = {
          green = {
            -1,
            3.34375
          },
          red = {
            -0.8125,
            3.34375
          }
        },
        wire = {
          green = {
            -0.9375,
            2.90625
          },
          red = {
            -1,
            2.6875
          }
        }
      },
      sprites = {
        blue_led_light_offset = {
          -1.25,
          2.9375
        },
        connector_main = {
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04a-base-sequence.png",
          height = 50,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.6875
          },
          width = 52,
          x = 104,
          y = 150
        },
        led_blue = {
          draw_as_glow = true,
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04e-blue-LED-on-sequence.png",
          height = 60,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.65625
          },
          width = 60,
          x = 120,
          y = 180
        },
        led_blue_off = {
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04f-blue-LED-off-sequence.png",
          height = 44,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.65625
          },
          width = 46,
          x = 92,
          y = 132
        },
        led_green = {
          draw_as_glow = true,
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04h-green-LED-sequence.png",
          height = 46,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.65625
          },
          width = 48,
          x = 96,
          y = 138
        },
        led_light = {
          intensity = 0,
          size = 0.9
        },
        led_red = {
          draw_as_glow = true,
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04i-red-LED-sequence.png",
          height = 46,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.65625
          },
          width = 48,
          x = 96,
          y = 138
        },
        red_green_led_light_offset = {
          -1.25,
          2.84375
        },
        wire_pins = {
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04c-wire-sequence.png",
          height = 58,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.6875
          },
          width = 62,
          x = 124,
          y = 174
        }
      }
    },
    {
      points = {
        shadow = {
          green = {
            -1,
            3.34375
          },
          red = {
            -0.8125,
            3.34375
          }
        },
        wire = {
          green = {
            -0.9375,
            2.90625
          },
          red = {
            -1,
            2.6875
          }
        }
      },
      sprites = {
        blue_led_light_offset = {
          -1.25,
          2.9375
        },
        connector_main = {
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04a-base-sequence.png",
          height = 50,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.6875
          },
          width = 52,
          x = 104,
          y = 150
        },
        led_blue = {
          draw_as_glow = true,
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04e-blue-LED-on-sequence.png",
          height = 60,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.65625
          },
          width = 60,
          x = 120,
          y = 180
        },
        led_blue_off = {
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04f-blue-LED-off-sequence.png",
          height = 44,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.65625
          },
          width = 46,
          x = 92,
          y = 132
        },
        led_green = {
          draw_as_glow = true,
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04h-green-LED-sequence.png",
          height = 46,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.65625
          },
          width = 48,
          x = 96,
          y = 138
        },
        led_light = {
          intensity = 0,
          size = 0.9
        },
        led_red = {
          draw_as_glow = true,
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04i-red-LED-sequence.png",
          height = 46,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.65625
          },
          width = 48,
          x = 96,
          y = 138
        },
        red_green_led_light_offset = {
          -1.25,
          2.84375
        },
        wire_pins = {
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04c-wire-sequence.png",
          height = 58,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.6875
          },
          width = 62,
          x = 124,
          y = 174
        }
      }
    },
    {
      points = {
        shadow = {
          green = {
            -1,
            3.34375
          },
          red = {
            -0.8125,
            3.34375
          }
        },
        wire = {
          green = {
            -0.9375,
            2.90625
          },
          red = {
            -1,
            2.6875
          }
        }
      },
      sprites = {
        blue_led_light_offset = {
          -1.25,
          2.9375
        },
        connector_main = {
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04a-base-sequence.png",
          height = 50,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.6875
          },
          width = 52,
          x = 104,
          y = 150
        },
        led_blue = {
          draw_as_glow = true,
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04e-blue-LED-on-sequence.png",
          height = 60,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.65625
          },
          width = 60,
          x = 120,
          y = 180
        },
        led_blue_off = {
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04f-blue-LED-off-sequence.png",
          height = 44,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.65625
          },
          width = 46,
          x = 92,
          y = 132
        },
        led_green = {
          draw_as_glow = true,
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04h-green-LED-sequence.png",
          height = 46,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.65625
          },
          width = 48,
          x = 96,
          y = 138
        },
        led_light = {
          intensity = 0,
          size = 0.9
        },
        led_red = {
          draw_as_glow = true,
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04i-red-LED-sequence.png",
          height = 46,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.65625
          },
          width = 48,
          x = 96,
          y = 138
        },
        red_green_led_light_offset = {
          -1.25,
          2.84375
        },
        wire_pins = {
          filename = "__base__/graphics/entity/circuit-connector/ccm-universal-04c-wire-sequence.png",
          height = 58,
          priority = "low",
          scale = 0.5,
          shift = {
            -1.25,
            2.6875
          },
          width = 62,
          x = 124,
          y = 174
        }
      }
    }
  },
  circuit_wire_max_distance = 9,
  close_sound = {
    filename = "__base__/sound/machine-close.ogg",
    volume = 0.5
  },
  collision_box = {
    {
      -4.0999999999999996,
      -4.0999999999999996
    },
    {
      4.0999999999999996,
      4.0999999999999996
    }
  },
  collision_mask = {
    layers = {
      caravan_collision_mask = true,
      is_lower_object = true,
      is_object = true,
      item = true,
      meltable = true,
      object = true,
      player = true,
      water_tile = true
    }
  },
  corpse = "big-remnants",
  dying_explosion = "big-explosion",
  energy_source = {
    emissions_per_minute = {
      pollution = 1
    },
    type = "electric",
    usage_priority = "secondary-input"
  },
  energy_usage = "420kW",
  fast_replaceable_group = "borax-mine",
  flags = {
    "placeable-neutral",
    "player-creation"
  },
  graphics_set = {
    animation = {
      layers = {
        {
          animation_speed = 0.1,
          filename = "__pycoalprocessinggraphics__/graphics/entity/borax-mine/bottom-borax-mk01.png",
          frame_count = 90,
          height = 144,
          line_length = 7,
          shift = {
            0,
            2.25
          },
          width = 288
        },
        {
          animation_speed = 0.1,
          filename = "__pycoalprocessinggraphics__/graphics/entity/borax-mine/top-borax-mk01.png",
          frame_count = 90,
          height = 144,
          line_length = 7,
          shift = {
            0,
            -2.25
          },
          width = 288
        }
      }
    }
  },
  icon = "__pycoalprocessinggraphics__/graphics/icons/borax-mine.png",
  icon_size = 64,
  icons_positioning = {
    {
      inventory_index = 2,
      max_icons_per_row = 1,
      scale = 1.5,
      shift = {
        0,
        2.8499999999999996
      }
    }
  },
  impact_category = "metal",
  input_fluid_box = {
    pipe_connections = {
      {
        connection_category = {
          "default",
          "pipe",
          "niobium-pipe",
          "ht-pipes"
        },
        direction = 12,
        position = {
          -4.0999999999999996,
          0
        }
      },
      {
        connection_category = {
          "default",
          "pipe",
          "niobium-pipe",
          "ht-pipes"
        },
        direction = 4,
        position = {
          4.0999999999999996,
          0
        }
      },
      {
        connection_category = {
          "default",
          "pipe",
          "niobium-pipe",
          "ht-pipes"
        },
        direction = 8,
        position = {
          0,
          4.0999999999999996
        }
      }
    },
    pipe_covers = {
      east = {
        layers = {
          {
            filename = "__base__/graphics/entity/pipe-covers/pipe-cover-east.png",
            height = 128,
            priority = "extra-high",
            scale = 0.5,
            width = 128
          },
          {
            draw_as_shadow = true,
            filename = "__base__/graphics/entity/pipe-covers/pipe-cover-east-shadow.png",
            height = 128,
            priority = "extra-high",
            scale = 0.5,
            width = 128
          }
        }
      },
      north = {
        layers = {
          {
            filename = "__base__/graphics/entity/pipe-covers/pipe-cover-north.png",
            height = 128,
            priority = "extra-high",
            scale = 0.5,
            width = 128
          },
          {
            draw_as_shadow = true,
            filename = "__base__/graphics/entity/pipe-covers/pipe-cover-north-shadow.png",
            height = 128,
            priority = "extra-high",
            scale = 0.5,
            width = 128
          }
        }
      },
      south = {
        layers = {
          {
            filename = "__base__/graphics/entity/pipe-covers/pipe-cover-south.png",
            height = 128,
            priority = "extra-high",
            scale = 0.5,
            width = 128
          },
          {
            draw_as_shadow = true,
            filename = "__base__/graphics/entity/pipe-covers/pipe-cover-south-shadow.png",
            height = 128,
            priority = "extra-high",
            scale = 0.5,
            width = 128
          }
        }
      },
      west = {
        layers = {
          {
            filename = "__base__/graphics/entity/pipe-covers/pipe-cover-west.png",
            height = 128,
            priority = "extra-high",
            scale = 0.5,
            width = 128
          },
          {
            draw_as_shadow = true,
            filename = "__base__/graphics/entity/pipe-covers/pipe-cover-west-shadow.png",
            height = 128,
            priority = "extra-high",
            scale = 0.5,
            width = 128
          }
        }
      }
    },
    pipe_picture = {
      east = {
        filename = "__base__/graphics/entity/assembling-machine-2/assembling-machine-2-pipe-E.png",
        height = 76,
        priority = "extra-high",
        scale = 0.5,
        shift = {
          -0.765625,
          0.03125
        },
        width = 42
      },
      north = {
        filename = "__base__/graphics/entity/assembling-machine-2/assembling-machine-2-pipe-N.png",
        height = 38,
        priority = "extra-high",
        scale = 0.5,
        shift = {
          0.0703125,
          0.421875
        },
        width = 71
      },
      south = {
        filename = "__base__/graphics/entity/assembling-machine-2/assembling-machine-2-pipe-S.png",
        height = 61,
        priority = "extra-high",
        scale = 0.5,
        shift = {
          0,
          -0.9765625
        },
        width = 88
      },
      west = {
        filename = "__base__/graphics/entity/assembling-machine-2/assembling-machine-2-pipe-W.png",
        height = 73,
        priority = "extra-high",
        scale = 0.5,
        shift = {
          0.8046875,
          0.0390625
        },
        width = 39
      }
    },
    volume = 200
  },
  max_health = 700,
  minable = {
    mining_time = 1,
    results = {
      {
        amount = 1,
        name = "borax-mine",
        type = "item"
      }
    }
  },
  mining_speed = 2,
  module_slots = 1,
  name = "borax-mine",
  next_upgrade = "borax-mine-mk02",
  open_sound = {
    filename = "__base__/sound/machine-open.ogg",
    volume = 0.5
  },
  perceived_performance = {
    maximum = 1.5,
    performance_to_activity_rate = 0.2
  },
  radius_visualisation_picture = {
    filename = "__base__/graphics/entity/electric-mining-drill/electric-mining-drill-radius-visualization.png",
    height = 12,
    width = 12
  },
  resource_categories = {
    "borax"
  },
  resource_drain_rate_percent = 100,
  resource_searching_radius = 5.4900000000000002,
  selection_box = {
    {
      -4.5,
      -4.5
    },
    {
      4.5,
      4.5
    }
  },
  type = "mining-drill",
  vector_to_place_result = {
    0,
    -4.6500000000000004
  },
  wet_mining_graphics_set = {
    animation = {
      east = {
        layers = {
          {
            animation_speed = 0.1,
            filename = "__pycoalprocessinggraphics__/graphics/entity/borax-mine/bottom-borax-mk01.png",
            frame_count = 90,
            height = 144,
            line_length = 7,
            shift = {
              0,
              2.25
            },
            width = 288
          },
          {
            animation_speed = 0.1,
            filename = "__pycoalprocessinggraphics__/graphics/entity/borax-mine/top-borax-mk01.png",
            frame_count = 90,
            height = 144,
            line_length = 7,
            shift = {
              0,
              -2.25
            },
            width = 288
          },
          {
            direction_count = 1,
            filename = "__pycoalprocessinggraphics__/graphics/entity/borax-mine/E.png",
            frame_count = 1,
            height = 288,
            line_length = 1,
            priority = "extra-high",
            repeat_count = 90,
            shift = {
              0,
              0
            },
            width = 288
          }
        }
      },
      north = {
        layers = {
          {
            animation_speed = 0.1,
            filename = "__pycoalprocessinggraphics__/graphics/entity/borax-mine/bottom-borax-mk01.png",
            frame_count = 90,
            height = 144,
            line_length = 7,
            shift = {
              0,
              2.25
            },
            width = 288
          },
          {
            animation_speed = 0.1,
            filename = "__pycoalprocessinggraphics__/graphics/entity/borax-mine/top-borax-mk01.png",
            frame_count = 90,
            height = 144,
            line_length = 7,
            shift = {
              0,
              -2.25
            },
            width = 288
          },
          {
            direction_count = 1,
            filename = "__pycoalprocessinggraphics__/graphics/entity/borax-mine/N.png",
            frame_count = 1,
            height = 288,
            line_length = 1,
            priority = "extra-high",
            repeat_count = 90,
            shift = {
              0,
              0
            },
            width = 288
          }
        }
      },
      south = {
        layers = {
          {
            animation_speed = 0.1,
            filename = "__pycoalprocessinggraphics__/graphics/entity/borax-mine/bottom-borax-mk01.png",
            frame_count = 90,
            height = 144,
            line_length = 7,
            shift = {
              0,
              2.25
            },
            width = 288
          },
          {
            animation_speed = 0.1,
            filename = "__pycoalprocessinggraphics__/graphics/entity/borax-mine/top-borax-mk01.png",
            frame_count = 90,
            height = 144,
            line_length = 7,
            shift = {
              0,
              -2.25
            },
            width = 288
          },
          {
            direction_count = 1,
            filename = "__pycoalprocessinggraphics__/graphics/entity/borax-mine/S.png",
            frame_count = 1,
            height = 288,
            line_length = 1,
            priority = "extra-high",
            repeat_count = 90,
            shift = {
              0,
              0
            },
            width = 288
          }
        }
      },
      west = {
        layers = {
          {
            animation_speed = 0.1,
            filename = "__pycoalprocessinggraphics__/graphics/entity/borax-mine/bottom-borax-mk01.png",
            frame_count = 90,
            height = 144,
            line_length = 7,
            shift = {
              0,
              2.25
            },
            width = 288
          },
          {
            animation_speed = 0.1,
            filename = "__pycoalprocessinggraphics__/graphics/entity/borax-mine/top-borax-mk01.png",
            frame_count = 90,
            height = 144,
            line_length = 7,
            shift = {
              0,
              -2.25
            },
            width = 288
          },
          {
            direction_count = 1,
            filename = "__pycoalprocessinggraphics__/graphics/entity/borax-mine/W.png",
            frame_count = 1,
            height = 288,
            line_length = 1,
            priority = "extra-high",
            repeat_count = 90,
            shift = {
              0,
              0
            },
            width = 288
          }
        }
      }
    }
  },
  working_sound = {
    apparent_volume = 2.5,
    idle_sound = {
      filename = "__pycoalprocessinggraphics__/sounds/borax-mine.ogg",
      volume = 0.3
    },
    sound = {
      filename = "__pycoalprocessinggraphics__/sounds/borax-mine.ogg"
    }
  }
}
